...
const board1 = document.querySelectorAll(".div");
  ...
  if (element.textContent == "") {
    ...
    if (!isWinner) {
      ...
      for (x = 0; x < board.length; x++) {
        let loopelement = document.getElementById(board[x].id);

        if (loopelement.textContent == "") {
          Competition();
          winner();
          return;
        }
...
for (let i = 0; i < board2.length; i++) {
  for (let j = 0; j < board2[i].length; j++) {
    board2[i][j].addEventListener("click", playGame);
  }
}
